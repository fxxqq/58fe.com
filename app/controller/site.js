...
class HomeController extends Controller {
  async index() {
    let page = parseInt(this.ctx.params.page, 10) || 1;
    page = page > 0 ? page : 1;
    const tab = this.ctx.params.tab || 'index';
    ...
    let tabIndex=0;
    this.config.tabs.forEach((tabItem, index) => {
      if (tab === tabItem.tab) {
        tabIndex = index;
      }
    });
    ...
    const locals = {
      ...
      tab,
      labels: this.config.tabs[tabIndex].labels,
      label,
      pageTitle: tabName,
    };
